...
unsigned long int
decode_uleb128(unsigned char *data, int *length_return)
{
...
signed long int
decode_sleb128(unsigned char *data, int *length_return)
{
...
int
check_CFI(unsigned char *ptr, int *error, int ptr_encoding)
{
    int numused, used = 0;
    int tmp;
    long tmpaddr;
...
int
check_FDE(CIEFrame * cie_list, unsigned char *ptr, int *error)
{
    int numused, used = 0;
...
*/
	...
	if (cie->fde_encoding) {
		...
		read_FDE_encoded(ptr, cie->fde_encoding, &numused);
	    ptr += numused;
	    used += numused;
	} else {
...
int
check_CIE(CIEFrame ** cie_list, unsigned char *ptr, int *error)
{
    /* CIEFrameImage *frameimg; */
    CIEFrame *frame;
    ...
    *cie_list = frame;
	...
	for (aug_char = 1; aug_char < strlen(frame->augmentation);
	     aug_char++) {
		...
		if (frame->per_encoding) {
			...
			read_FDE_encoded(ptr, frame->per_encoding,
					 &numused);
		    ptr += numused;
		} else {
