...
void XShapeCombineRectangles (Display * arg0 , Window arg1 , int arg2 , int arg3 , int arg4 , XRectangle * arg5 , int arg6 , int arg7 , int arg8 )
{
	...
	__lsb_output(4, "Invoking wrapper for XShapeCombineRectangles()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XShapeCombineRectangles");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XShapeCombineRectangles. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
