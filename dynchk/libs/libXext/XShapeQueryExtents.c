...
int XShapeQueryExtents (Display * arg0 , Window arg1 , int * arg2 , int * arg3 , int * arg4 , unsigned int * arg5 , unsigned int * arg6 , int * arg7 , int * arg8 , int * arg9 , unsigned int * arg10 , unsigned int * arg11 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XShapeQueryExtents()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XShapeQueryExtents");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XShapeQueryExtents. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
