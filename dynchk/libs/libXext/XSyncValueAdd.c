...
void XSyncValueAdd (XSyncValue * arg0 , XSyncValue arg1 , XSyncValue arg2 , int * arg3 )
{
	...
	__lsb_output(4, "Invoking wrapper for XSyncValueAdd()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XSyncValueAdd");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XSyncValueAdd. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
