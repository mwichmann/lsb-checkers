...
int XSyncValueLessOrEqual (XSyncValue arg0 , XSyncValue arg1 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XSyncValueLessOrEqual()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XSyncValueLessOrEqual");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XSyncValueLessOrEqual. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XSyncValueLessOrEqual() - validating");
