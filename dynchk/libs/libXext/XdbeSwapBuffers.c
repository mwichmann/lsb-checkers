// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <X11/Xlib.h>
#include <X11/extensions/Xdbe.h>
#undef XdbeSwapBuffers
static int(*funcptr) (Display * , XdbeSwapInfo * , int ) = 0;

extern int __lsb_check_params;
int XdbeSwapBuffers (Display * arg0 , XdbeSwapInfo * arg1 , int arg2 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XdbeSwapBuffers()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XdbeSwapBuffers");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XdbeSwapBuffers. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XdbeSwapBuffers() - validating");
		if( arg0 ) {
		validate_RWaddress( arg0, "XdbeSwapBuffers - arg0");
		}
		validate_NULL_TYPETYPE(  arg0, "XdbeSwapBuffers - arg0");
		if( arg1 ) {
		validate_RWaddress( arg1, "XdbeSwapBuffers - arg1");
		}
		validate_NULL_TYPETYPE(  arg1, "XdbeSwapBuffers - arg1");
		validate_NULL_TYPETYPE(  arg2, "XdbeSwapBuffers - arg2");
	}
	ret_value = funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
	return ret_value;
}

