...
int XSyncQueryExtension (Display * arg0 , int * arg1 , int * arg2 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XSyncQueryExtension()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XSyncQueryExtension");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XSyncQueryExtension. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
