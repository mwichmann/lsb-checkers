...
void XShapeCombineRegion (Display * arg0 , Window arg1 , int arg2 , int arg3 , int arg4 , Region arg5 , int arg6 )
{
	...
	__lsb_output(4, "Invoking wrapper for XShapeCombineRegion()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XShapeCombineRegion");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XShapeCombineRegion. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
