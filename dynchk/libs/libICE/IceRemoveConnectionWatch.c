// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <X11/ICE/ICElib.h>
#undef IceRemoveConnectionWatch
static void(*funcptr) (IceWatchProc , IcePointer ) = 0;

extern int __lsb_check_params;
void IceRemoveConnectionWatch (IceWatchProc arg0 , IcePointer arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for IceRemoveConnectionWatch()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "IceRemoveConnectionWatch");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load IceRemoveConnectionWatch. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "IceRemoveConnectionWatch() - validating");
		validate_NULL_TYPETYPE(  arg0, "IceRemoveConnectionWatch - arg0");
		validate_NULL_TYPETYPE(  arg1, "IceRemoveConnectionWatch - arg1");
	}
	funcptr(arg0, arg1);
	__lsb_check_params = reset_flag;
}

