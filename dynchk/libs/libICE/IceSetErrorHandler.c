// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <X11/ICE/ICElib.h>
#undef IceSetErrorHandler
static IceErrorHandler(*funcptr) (IceErrorHandler ) = 0;

extern int __lsb_check_params;
IceErrorHandler IceSetErrorHandler (IceErrorHandler arg0 )
{
	int reset_flag = __lsb_check_params;
	IceErrorHandler ret_value  ;
	__lsb_output(4, "Invoking wrapper for IceSetErrorHandler()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "IceSetErrorHandler");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load IceSetErrorHandler. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "IceSetErrorHandler() - validating");
		validate_NULL_TYPETYPE(  arg0, "IceSetErrorHandler - arg0");
	}
	ret_value = funcptr(arg0);
	__lsb_check_params = reset_flag;
	return ret_value;
}

