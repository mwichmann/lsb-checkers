...
{
	my($fh, $func_id, $func_name, $func_type_string, $is_lsb) = @_;
	my $i = 0; my $j = 0;
	...
	VALCALL: while( my($param_pos, $param_int) = $write_int_wrapper_q->fetchrow_array() )
	{
		...
		next VALCALL if(get_param_type($param_pos, $param_int) eq "void");
	...
	print $fh "\t";
	print $fh "return " unless($func_type_string eq "void");
	print $fh "funcptr(";
...
sub write_argument_list # ($fh, $func_id, $add_arg)
{
	...
	{
		$type = get_param_type($param_pos, $param_int);
		unless($type eq "void")
		{
			print $fh ", " unless($i == 0);
			print $fh $type;
...
sub write_int_declaration # ($fh, $func_id, $func_name, $func_type_string, $is_lsb)
{
	my($fh, $func_id, $func_name, $func_type_string, $is_lsb) = @_;
	print $fh $func_type_string . " ";
...
FUNC: while(my ($func_id, $func_name, $func_lib, $func_type) = $interface_q->fetchrow_array())
{
	...
	my $type_string = get_type_string($func_type);
	...
	write_int_wrapper($interface_file, $func_id, $func_name, $type_string, 1);
