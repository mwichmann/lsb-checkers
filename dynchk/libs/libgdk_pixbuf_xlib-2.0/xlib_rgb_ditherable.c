...
int xlib_rgb_ditherable ()
{
	...
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xlib_rgb_ditherable. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "xlib_rgb_ditherable() - validating");
	}
	ret_value = funcptr();
