...
int XSetBackground (Display * arg0 , GC arg1 , unsigned long int arg2 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XSetBackground()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XSetBackground");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XSetBackground. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XSetBackground() - validating");
