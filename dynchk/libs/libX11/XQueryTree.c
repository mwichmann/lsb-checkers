// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <X11/Xlib.h>
#include <X11/X.h>
#undef XQueryTree
static int(*funcptr) (Display * , Window , Window * , Window * , Window * * , unsigned int * ) = 0;

extern int __lsb_check_params;
int XQueryTree (Display * arg0 , Window arg1 , Window * arg2 , Window * arg3 , Window * * arg4 , unsigned int * arg5 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XQueryTree()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XQueryTree");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XQueryTree. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XQueryTree() - validating");
		validate_RWaddress( arg0, "XQueryTree - arg0");
		validate_NULL_TYPETYPE(  arg0, "XQueryTree - arg0");
		validate_NULL_TYPETYPE(  arg1, "XQueryTree - arg1");
		validate_RWaddress( arg2, "XQueryTree - arg2");
		validate_NULL_TYPETYPE(  arg2, "XQueryTree - arg2");
		validate_RWaddress( arg3, "XQueryTree - arg3");
		validate_NULL_TYPETYPE(  arg3, "XQueryTree - arg3");
		validate_RWaddress( arg4, "XQueryTree - arg4");
		validate_NULL_TYPETYPE(  arg4, "XQueryTree - arg4");
		validate_RWaddress( arg5, "XQueryTree - arg5");
		validate_NULL_TYPETYPE(  arg5, "XQueryTree - arg5");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3, arg4, arg5);
	__lsb_check_params = reset_flag;
	return ret_value;
}

