// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <X11/Xlib.h>
#include <X11/X.h>
#include <X11/Xutil.h>
#undef XLookupString
static int(*funcptr) (XKeyEvent * , char * , int , KeySym * , XComposeStatus * ) = 0;

extern int __lsb_check_params;
int XLookupString (XKeyEvent * arg0 , char * arg1 , int arg2 , KeySym * arg3 , XComposeStatus * arg4 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XLookupString()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XLookupString");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XLookupString. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XLookupString() - validating");
		validate_RWaddress( arg0, "XLookupString - arg0");
		validate_NULL_TYPETYPE(  arg0, "XLookupString - arg0");
		validate_RWaddress( arg1, "XLookupString - arg1");
		validate_NULL_TYPETYPE(  arg1, "XLookupString - arg1");
		validate_NULL_TYPETYPE(  arg2, "XLookupString - arg2");
		validate_RWaddress( arg3, "XLookupString - arg3");
		validate_NULL_TYPETYPE(  arg3, "XLookupString - arg3");
		validate_RWaddress( arg4, "XLookupString - arg4");
		validate_NULL_TYPETYPE(  arg4, "XLookupString - arg4");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3, arg4);
	__lsb_check_params = reset_flag;
	return ret_value;
}

