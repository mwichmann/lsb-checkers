// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <X11/Xlib.h>
#include <X11/X.h>
#include <X11/XKBlib.h>
#undef XkbLookupKeyBinding
static int(*funcptr) (Display * , KeySym , unsigned int , char * , int , int * ) = 0;

extern int __lsb_check_params;
int XkbLookupKeyBinding (Display * arg0 , KeySym arg1 , unsigned int arg2 , char * arg3 , int arg4 , int * arg5 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XkbLookupKeyBinding()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XkbLookupKeyBinding");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XkbLookupKeyBinding. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XkbLookupKeyBinding() - validating");
		validate_RWaddress( arg0, "XkbLookupKeyBinding - arg0");
		validate_NULL_TYPETYPE(  arg0, "XkbLookupKeyBinding - arg0");
		validate_NULL_TYPETYPE(  arg1, "XkbLookupKeyBinding - arg1");
		validate_NULL_TYPETYPE(  arg2, "XkbLookupKeyBinding - arg2");
		validate_RWaddress( arg3, "XkbLookupKeyBinding - arg3");
		validate_NULL_TYPETYPE(  arg3, "XkbLookupKeyBinding - arg3");
		validate_NULL_TYPETYPE(  arg4, "XkbLookupKeyBinding - arg4");
		validate_RWaddress( arg5, "XkbLookupKeyBinding - arg5");
		validate_NULL_TYPETYPE(  arg5, "XkbLookupKeyBinding - arg5");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3, arg4, arg5);
	__lsb_check_params = reset_flag;
	return ret_value;
}

