...
int XLookupColor (Display * arg0 , Colormap arg1 , const char * arg2 , XColor * arg3 , XColor * arg4 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XLookupColor()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XLookupColor");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XLookupColor. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XLookupColor() - validating");
