...
int XTranslateCoordinates (Display * arg0 , Window arg1 , Window arg2 , int arg3 , int arg4 , int * arg5 , int * arg6 , Window * arg7 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XTranslateCoordinates()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XTranslateCoordinates");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XTranslateCoordinates. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XTranslateCoordinates() - validating");
