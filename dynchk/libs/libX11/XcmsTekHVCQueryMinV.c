...
int XcmsTekHVCQueryMinV (XcmsCCC arg0 , XcmsFloat arg1 , XcmsFloat arg2 , XcmsColor * arg3 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XcmsTekHVCQueryMinV()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XcmsTekHVCQueryMinV");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XcmsTekHVCQueryMinV. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XcmsTekHVCQueryMinV() - validating");
