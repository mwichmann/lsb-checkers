...
char * XScreenResourceString (Screen * arg0 )
{
	...
	char * ret_value  ;
	__lsb_output(4, "Invoking wrapper for XScreenResourceString()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XScreenResourceString");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XScreenResourceString. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XScreenResourceString() - validating");
