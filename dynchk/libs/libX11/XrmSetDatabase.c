...
void XrmSetDatabase (Display * arg0 , XrmDatabase arg1 )
{
	...
	__lsb_output(4, "Invoking wrapper for XrmSetDatabase()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XrmSetDatabase");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XrmSetDatabase. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XrmSetDatabase() - validating");
