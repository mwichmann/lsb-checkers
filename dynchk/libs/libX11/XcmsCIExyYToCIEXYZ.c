...
int XcmsCIExyYToCIEXYZ (XcmsCCC arg0 , XcmsColor * arg1 , XcmsColor * arg2 , unsigned int arg3 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XcmsCIExyYToCIEXYZ()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XcmsCIExyYToCIEXYZ");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XcmsCIExyYToCIEXYZ. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XcmsCIExyYToCIEXYZ() - validating");
