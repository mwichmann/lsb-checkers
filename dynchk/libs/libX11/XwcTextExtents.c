...
int XwcTextExtents (XFontSet arg0 , const wchar_t * arg1 , int arg2 , XRectangle * arg3 , XRectangle * arg4 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XwcTextExtents()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XwcTextExtents");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XwcTextExtents. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XwcTextExtents() - validating");
