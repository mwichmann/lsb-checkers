...
int XQueryColors (Display * arg0 , Colormap arg1 , XColor * arg2 , int arg3 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XQueryColors()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XQueryColors");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XQueryColors. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XQueryColors() - validating");
