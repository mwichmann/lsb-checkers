...
int XDrawArc (Display * arg0 , Drawable arg1 , GC arg2 , int arg3 , int arg4 , unsigned int arg5 , unsigned int arg6 , int arg7 , int arg8 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XDrawArc()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XDrawArc");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XDrawArc. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XDrawArc() - validating");
