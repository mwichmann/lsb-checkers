...
void XkbFreeGeomKeys (XkbRowPtr arg0 , int arg1 , int arg2 , int arg3 )
{
	...
	__lsb_output(4, "Invoking wrapper for XkbFreeGeomKeys()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XkbFreeGeomKeys");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XkbFreeGeomKeys. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XkbFreeGeomKeys() - validating");
