...
int XTextExtents (XFontStruct * arg0 , const char * arg1 , int arg2 , int * arg3 , int * arg4 , int * arg5 , XCharStruct * arg6 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XTextExtents()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XTextExtents");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XTextExtents. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XTextExtents() - validating");
