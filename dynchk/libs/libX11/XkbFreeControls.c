...
void XkbFreeControls (XkbDescPtr arg0 , unsigned int arg1 , int arg2 )
{
	...
	__lsb_output(4, "Invoking wrapper for XkbFreeControls()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XkbFreeControls");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XkbFreeControls. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XkbFreeControls() - validating");
