// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <X11/Xlib.h>
#include <stddef.h>
#include <X11/X.h>
#undef XwcLookupString
static int(*funcptr) (XIC , XKeyPressedEvent * , wchar_t * , int , KeySym * , int * ) = 0;

extern int __lsb_check_params;
int XwcLookupString (XIC arg0 , XKeyPressedEvent * arg1 , wchar_t * arg2 , int arg3 , KeySym * arg4 , int * arg5 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XwcLookupString()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XwcLookupString");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XwcLookupString. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XwcLookupString() - validating");
		validate_NULL_TYPETYPE(  arg0, "XwcLookupString - arg0");
		validate_RWaddress( arg1, "XwcLookupString - arg1");
		validate_NULL_TYPETYPE(  arg1, "XwcLookupString - arg1");
		validate_RWaddress( arg2, "XwcLookupString - arg2");
		validate_NULL_TYPETYPE(  arg2, "XwcLookupString - arg2");
		validate_NULL_TYPETYPE(  arg3, "XwcLookupString - arg3");
		validate_RWaddress( arg4, "XwcLookupString - arg4");
		validate_NULL_TYPETYPE(  arg4, "XwcLookupString - arg4");
		validate_RWaddress( arg5, "XwcLookupString - arg5");
		validate_NULL_TYPETYPE(  arg5, "XwcLookupString - arg5");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3, arg4, arg5);
	__lsb_check_params = reset_flag;
	return ret_value;
}

