...
int XStoreBytes (Display * arg0 , const char * arg1 , int arg2 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for XStoreBytes()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XStoreBytes");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XStoreBytes. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XStoreBytes() - validating");
