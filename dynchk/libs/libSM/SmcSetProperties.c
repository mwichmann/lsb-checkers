...
void SmcSetProperties (SmcConn arg0 , int arg1 , SmProp * * arg2 )
{
	...
	__lsb_output(4, "Invoking wrapper for SmcSetProperties()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "SmcSetProperties");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load SmcSetProperties. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "SmcSetProperties() - validating");
