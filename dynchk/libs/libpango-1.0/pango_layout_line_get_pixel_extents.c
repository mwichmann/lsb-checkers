...
#undef pango_layout_line_get_pixel_extents
static void(*funcptr) (PangoLayoutLine * , PangoRectangle * , PangoRectangle * ) = 0;

extern int __lsb_check_params;
void pango_layout_line_get_pixel_extents (PangoLayoutLine * arg0 , PangoRectangle * arg1 , PangoRectangle * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for pango_layout_line_get_pixel_extents()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "pango_layout_line_get_pixel_extents");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load pango_layout_line_get_pixel_extents. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
