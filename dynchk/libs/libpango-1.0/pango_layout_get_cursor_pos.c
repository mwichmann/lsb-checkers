...
#undef pango_layout_get_cursor_pos
static void(*funcptr) (PangoLayout * , int , PangoRectangle * , PangoRectangle * ) = 0;

extern int __lsb_check_params;
void pango_layout_get_cursor_pos (PangoLayout * arg0 , int arg1 , PangoRectangle * arg2 , PangoRectangle * arg3 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for pango_layout_get_cursor_pos()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "pango_layout_get_cursor_pos");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load pango_layout_get_cursor_pos. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
