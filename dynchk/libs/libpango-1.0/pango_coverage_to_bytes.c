...
void pango_coverage_to_bytes (PangoCoverage * arg0 , guchar * * arg1 , int * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for pango_coverage_to_bytes()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "pango_coverage_to_bytes");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load pango_coverage_to_bytes. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
