...
#undef pango_layout_get_size
static void(*funcptr) (PangoLayout * , int * , int * ) = 0;

extern int __lsb_check_params;
void pango_layout_get_size (PangoLayout * arg0 , int * arg1 , int * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for pango_layout_get_size()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "pango_layout_get_size");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load pango_layout_get_size. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
