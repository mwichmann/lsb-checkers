...
#undef pango_parse_markup
static gboolean(*funcptr) (const char * , int , gunichar , PangoAttrList * * , char * * , gunichar * , GError * * ) = 0;

extern int __lsb_check_params;
gboolean pango_parse_markup (const char * arg0 , int arg1 , gunichar arg2 , PangoAttrList * * arg3 , char * * arg4 , gunichar * arg5 , GError * * arg6 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for pango_parse_markup()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "pango_parse_markup");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load pango_parse_markup. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
