// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <fontconfig/fontconfig.h>
#undef FcStrCmp
static int(*funcptr) (const FcChar8 * , const FcChar8 * ) = 0;

extern int __lsb_check_params;
int FcStrCmp (const FcChar8 * arg0 , const FcChar8 * arg1 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for FcStrCmp()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "FcStrCmp");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load FcStrCmp. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "FcStrCmp() - validating");
		if( arg0 ) {
		validate_Rdaddress( arg0, "FcStrCmp - arg0 (s1)");
		}
		validate_NULL_TYPETYPE(  arg0, "FcStrCmp - arg0 (s1)");
		if( arg1 ) {
		validate_Rdaddress( arg1, "FcStrCmp - arg1 (s2)");
		}
		validate_NULL_TYPETYPE(  arg1, "FcStrCmp - arg1 (s2)");
	}
	ret_value = funcptr(arg0, arg1);
	__lsb_check_params = reset_flag;
	return ret_value;
}

