...
#undef atk_editable_text_cut_text
static void(*funcptr) (AtkEditableText * , gint , gint ) = 0;

extern int __lsb_check_params;
void atk_editable_text_cut_text (AtkEditableText * arg0 , gint arg1 , gint arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for atk_editable_text_cut_text()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "atk_editable_text_cut_text");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load atk_editable_text_cut_text. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
