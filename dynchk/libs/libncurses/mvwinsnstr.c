...
int mvwinsnstr (WINDOW * arg0 , int arg1 , int arg2 , const char * arg3 , int arg4 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for mvwinsnstr()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "mvwinsnstr");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load mvwinsnstr. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
