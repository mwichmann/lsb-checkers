...
int wechochar (WINDOW * arg0 , const chtype arg1 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for wechochar()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "wechochar");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load wechochar. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "wechochar() - validating");
		if( arg0 ) {
