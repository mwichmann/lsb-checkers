...
int wbkgd (WINDOW * arg0 , chtype arg1 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for wbkgd()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "wbkgd");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load wbkgd. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "wbkgd() - validating");
