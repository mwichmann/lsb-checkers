...
chtype getbkgd (WINDOW * arg0 )
{
	...
	chtype ret_value  ;
	__lsb_output(4, "Invoking wrapper for getbkgd()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "getbkgd");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load getbkgd. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
