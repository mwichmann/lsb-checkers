...
int addchstr (const chtype * arg0 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for addchstr()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "addchstr");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load addchstr. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
