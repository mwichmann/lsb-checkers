...
int mvinchnstr (int arg0 , int arg1 , chtype * arg2 , int arg3 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for mvinchnstr()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "mvinchnstr");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load mvinchnstr. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
