...
int border (chtype arg0 , chtype arg1 , chtype arg2 , chtype arg3 , chtype arg4 , chtype arg5 , chtype arg6 , chtype arg7 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for border()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "border");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load border. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "border() - validating");
