...
int keypad (WINDOW * arg0 , bool arg1 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for keypad()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "keypad");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load keypad. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "keypad() - validating");
