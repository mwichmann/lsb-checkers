...
int nodelay (WINDOW * arg0 , bool arg1 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for nodelay()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "nodelay");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load nodelay. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
