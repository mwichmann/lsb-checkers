...
int addnstr (const char * arg0 , int arg1 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for addnstr()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "addnstr");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load addnstr. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "addnstr() - validating");
		if( arg0 ) {
