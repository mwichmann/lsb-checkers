...
int mvwchgat (WINDOW * arg0 , int arg1 , int arg2 , int arg3 , attr_t arg4 , short arg5 , const void * arg6 )
{
	...
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for mvwchgat()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "mvwchgat");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load mvwchgat. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "mvwchgat() - validating");
