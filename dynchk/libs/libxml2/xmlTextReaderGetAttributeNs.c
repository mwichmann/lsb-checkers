...
static xmlChar *(*funcptr) (xmlTextReaderPtr , const xmlChar * , const xmlChar * ) = 0;

extern int __lsb_check_params;
xmlChar * xmlTextReaderGetAttributeNs (xmlTextReaderPtr arg0 , const xmlChar * arg1 , const xmlChar * arg2 )
{
	int reset_flag = __lsb_check_params;
	xmlChar * ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlTextReaderGetAttributeNs()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlTextReaderGetAttributeNs", "LIBXML2_2.5.0");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlTextReaderGetAttributeNs. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
