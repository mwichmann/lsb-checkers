...
static xmlTextReaderPtr(*funcptr) (const char * , const char * , int ) = 0;

extern int __lsb_check_params;
xmlTextReaderPtr xmlReaderForFile (const char * arg0 , const char * arg1 , int arg2 )
{
	int reset_flag = __lsb_check_params;
	xmlTextReaderPtr ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlReaderForFile()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlReaderForFile", "LIBXML2_2.6.0");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlReaderForFile. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
