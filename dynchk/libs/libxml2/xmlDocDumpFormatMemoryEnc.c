...
static void(*funcptr) (xmlDocPtr , xmlChar * * , int * , const char * , int ) = 0;

extern int __lsb_check_params;
void xmlDocDumpFormatMemoryEnc (xmlDocPtr arg0 , xmlChar * * arg1 , int * arg2 , const char * arg3 , int arg4 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for xmlDocDumpFormatMemoryEnc()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlDocDumpFormatMemoryEnc", "LIBXML2_2.4.30");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlDocDumpFormatMemoryEnc. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
