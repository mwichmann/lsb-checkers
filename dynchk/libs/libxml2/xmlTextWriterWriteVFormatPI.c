// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <libxml2/libxml/xmlwriter.h>
#include <libxml2/libxml/xmlstring.h>
#include <stdarg.h>
#undef xmlTextWriterWriteVFormatPI
static int(*funcptr) (xmlTextWriterPtr , const xmlChar * , const char * , va_list ) = 0;

extern int __lsb_check_params;
int xmlTextWriterWriteVFormatPI (xmlTextWriterPtr arg0 , const xmlChar * arg1 , const char * arg2 , va_list arg3 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlTextWriterWriteVFormatPI()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlTextWriterWriteVFormatPI", "LIBXML2_2.6.0");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlTextWriterWriteVFormatPI. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "xmlTextWriterWriteVFormatPI() - validating");
		validate_NULL_TYPETYPE(  arg0, "xmlTextWriterWriteVFormatPI - arg0 (writer)");
		validate_Rdaddress( arg1, "xmlTextWriterWriteVFormatPI - arg1 (target)");
		validate_NULL_TYPETYPE(  arg1, "xmlTextWriterWriteVFormatPI - arg1 (target)");
		validate_Rdaddress( arg2, "xmlTextWriterWriteVFormatPI - arg2 (format)");
		validate_NULL_TYPETYPE(  arg2, "xmlTextWriterWriteVFormatPI - arg2 (format)");
		validate_NULL_TYPETYPE(  arg3, "xmlTextWriterWriteVFormatPI - arg3 (argptr)");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3);
	__lsb_check_params = reset_flag;
	return ret_value;
}

