// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <libxml2/libxml/tree.h>
#include <libxml2/libxml/xmlregexp.h>
#include <libxml2/libxml/valid.h>
#undef xmlValidGetValidElements
static int(*funcptr) (xmlNode * , xmlNode * , const xmlChar * * , int ) = 0;

extern int __lsb_check_params;
int xmlValidGetValidElements (xmlNode * arg0 , xmlNode * arg1 , const xmlChar * * arg2 , int arg3 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlValidGetValidElements()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlValidGetValidElements", "LIBXML2_2.4.30");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlValidGetValidElements. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "xmlValidGetValidElements() - validating");
		validate_RWaddress( arg0, "xmlValidGetValidElements - arg0");
		validate_NULL_TYPETYPE(  arg0, "xmlValidGetValidElements - arg0");
		validate_RWaddress( arg1, "xmlValidGetValidElements - arg1");
		validate_NULL_TYPETYPE(  arg1, "xmlValidGetValidElements - arg1");
		validate_RWaddress( arg2, "xmlValidGetValidElements - arg2");
		validate_NULL_TYPETYPE(  arg2, "xmlValidGetValidElements - arg2");
		validate_NULL_TYPETYPE(  arg3, "xmlValidGetValidElements - arg3");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3);
	__lsb_check_params = reset_flag;
	return ret_value;
}

