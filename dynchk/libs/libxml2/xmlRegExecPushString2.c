// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <libxml2/libxml/xmlregexp.h>
#include <libxml2/libxml/xmlstring.h>
#undef xmlRegExecPushString2
static int(*funcptr) (xmlRegExecCtxtPtr , const xmlChar * , const xmlChar * , void * ) = 0;

extern int __lsb_check_params;
int xmlRegExecPushString2 (xmlRegExecCtxtPtr arg0 , const xmlChar * arg1 , const xmlChar * arg2 , void * arg3 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlRegExecPushString2()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlRegExecPushString2", "LIBXML2_2.5.7");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlRegExecPushString2. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "xmlRegExecPushString2() - validating");
		validate_NULL_TYPETYPE(  arg0, "xmlRegExecPushString2 - arg0 (exec)");
		if( arg1 ) {
		validate_Rdaddress( arg1, "xmlRegExecPushString2 - arg1 (value)");
		}
		validate_NULL_TYPETYPE(  arg1, "xmlRegExecPushString2 - arg1 (value)");
		if( arg2 ) {
		validate_Rdaddress( arg2, "xmlRegExecPushString2 - arg2 (value2)");
		}
		validate_NULL_TYPETYPE(  arg2, "xmlRegExecPushString2 - arg2 (value2)");
		if( arg3 ) {
		validate_RWaddress( arg3, "xmlRegExecPushString2 - arg3 (data)");
		}
		validate_NULL_TYPETYPE(  arg3, "xmlRegExecPushString2 - arg3 (data)");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3);
	__lsb_check_params = reset_flag;
	return ret_value;
}

