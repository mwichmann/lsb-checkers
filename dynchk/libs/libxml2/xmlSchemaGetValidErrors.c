...
static int(*funcptr) (xmlSchemaValidCtxtPtr , xmlSchemaValidityErrorFunc * , xmlSchemaValidityWarningFunc * , void * * ) = 0;

extern int __lsb_check_params;
int xmlSchemaGetValidErrors (xmlSchemaValidCtxtPtr arg0 , xmlSchemaValidityErrorFunc * arg1 , xmlSchemaValidityWarningFunc * arg2 , void * * arg3 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlSchemaGetValidErrors()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlSchemaGetValidErrors", "LIBXML2_2.6.12");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlSchemaGetValidErrors. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
