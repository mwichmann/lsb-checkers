// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <libxml2/libxml/xmlstring.h>
#include <libxml2/libxml/dict.h>
#include <libxml2/libxml/xmlregexp.h>
#include <libxml2/libxml/pattern.h>
#undef xmlPatterncompile
static xmlPatternPtr(*funcptr) (const xmlChar * , xmlDict * , int , const xmlChar * * ) = 0;

extern int __lsb_check_params;
xmlPatternPtr xmlPatterncompile (const xmlChar * arg0 , xmlDict * arg1 , int arg2 , const xmlChar * * arg3 )
{
	int reset_flag = __lsb_check_params;
	xmlPatternPtr ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlPatterncompile()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlPatterncompile", "LIBXML2_2.6.3");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlPatterncompile. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "xmlPatterncompile() - validating");
		validate_Rdaddress( arg0, "xmlPatterncompile - arg0 (pattern)");
		validate_NULL_TYPETYPE(  arg0, "xmlPatterncompile - arg0 (pattern)");
		validate_RWaddress( arg1, "xmlPatterncompile - arg1 (dict)");
		validate_NULL_TYPETYPE(  arg1, "xmlPatterncompile - arg1 (dict)");
		validate_NULL_TYPETYPE(  arg2, "xmlPatterncompile - arg2 (flags)");
		validate_RWaddress( arg3, "xmlPatterncompile - arg3 (namespaces)");
		validate_NULL_TYPETYPE(  arg3, "xmlPatterncompile - arg3 (namespaces)");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3);
	__lsb_check_params = reset_flag;
	return ret_value;
}

