...
xmlNodePtr xmlStringGetNodeList (xmlDocPtr arg0 , const xmlChar * arg1 )
{
	int reset_flag = __lsb_check_params;
	xmlNodePtr ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlStringGetNodeList()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlStringGetNodeList", "LIBXML2_2.4.30");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlStringGetNodeList. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
