...
int xmlShellList (xmlShellCtxtPtr arg0 , char * arg1 , xmlNodePtr arg2 , xmlNodePtr arg3 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for xmlShellList()");
	if(!funcptr)
		#if 1
			funcptr = dlvsym(RTLD_NEXT, "xmlShellList", "LIBXML2_2.4.30");
		#endif
	if(!funcptr) {
		__lsb_output(-1, "Failed to load xmlShellList. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
