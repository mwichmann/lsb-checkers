...
const GDBusMethodInfo * g_dbus_method_invocation_get_method_info (GDBusMethodInvocation * arg0 )
{
	...
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_dbus_method_invocation_get_method_info. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "g_dbus_method_invocation_get_method_info() - validating");
		if( arg0 ) {
		validate_RWaddress( arg0, "g_dbus_method_invocation_get_method_info - arg0 (invocation)");
		}
		validate_NULL_TYPETYPE(  arg0, "g_dbus_method_invocation_get_method_info - arg0 (invocation)");
	}
