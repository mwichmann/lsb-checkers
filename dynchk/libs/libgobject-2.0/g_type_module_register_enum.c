...
static GType(*funcptr) (GTypeModule * , const gchar * , const GEnumValue * ) = 0;

extern int __lsb_check_params;
GType g_type_module_register_enum (GTypeModule * arg0 , const gchar * arg1 , const GEnumValue * arg2 )
{
	int reset_flag = __lsb_check_params;
	GType ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_type_module_register_enum()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_type_module_register_enum");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_type_module_register_enum. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
