...
void g_object_class_install_property (GObjectClass * arg0 , guint arg1 , GParamSpec * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for g_object_class_install_property()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_object_class_install_property");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_object_class_install_property. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
