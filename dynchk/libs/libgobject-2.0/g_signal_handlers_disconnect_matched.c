...
guint g_signal_handlers_disconnect_matched (gpointer arg0 , GSignalMatchType arg1 , guint arg2 , GQuark arg3 , GClosure * arg4 , gpointer arg5 , gpointer arg6 )
{
	int reset_flag = __lsb_check_params;
	guint ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_signal_handlers_disconnect_matched()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_signal_handlers_disconnect_matched");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_signal_handlers_disconnect_matched. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
