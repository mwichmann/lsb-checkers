...
void gdk_window_resize (GdkWindow * arg0 , gint arg1 , gint arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gdk_window_resize()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gdk_window_resize");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gdk_window_resize. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
