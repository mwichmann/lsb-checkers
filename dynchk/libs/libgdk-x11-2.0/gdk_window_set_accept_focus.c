...
void gdk_window_set_accept_focus (GdkWindow * arg0 , gboolean arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gdk_window_set_accept_focus()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gdk_window_set_accept_focus");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gdk_window_set_accept_focus. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
