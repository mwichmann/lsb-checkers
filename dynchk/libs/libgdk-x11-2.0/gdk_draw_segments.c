...
static void(*funcptr) (GdkDrawable * , GdkGC * , const GdkSegment * , gint ) = 0;

extern int __lsb_check_params;
void gdk_draw_segments (GdkDrawable * arg0 , GdkGC * arg1 , const GdkSegment * arg2 , gint arg3 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gdk_draw_segments()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gdk_draw_segments");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gdk_draw_segments. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
