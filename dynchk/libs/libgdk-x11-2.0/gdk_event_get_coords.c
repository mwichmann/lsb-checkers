...
gboolean gdk_event_get_coords (const GdkEvent * arg0 , gdouble * arg1 , gdouble * arg2 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gdk_event_get_coords()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gdk_event_get_coords");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gdk_event_get_coords. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
