...
GList * g_completion_complete_utf8 (GCompletion * arg0 , const gchar * arg1 , gchar * * arg2 )
{
	int reset_flag = __lsb_check_params;
	GList * ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_completion_complete_utf8()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_completion_complete_utf8");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_completion_complete_utf8. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
