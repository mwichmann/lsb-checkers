// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <glib-2.0/glib.h>
#undef g_ascii_formatd
static gchar *(*funcptr) (gchar * , gint , const gchar * , gdouble ) = 0;

extern int __lsb_check_params;
gchar * g_ascii_formatd (gchar * arg0 , gint arg1 , const gchar * arg2 , gdouble arg3 )
{
	int reset_flag = __lsb_check_params;
	gchar * ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_ascii_formatd()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_ascii_formatd");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_ascii_formatd. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "g_ascii_formatd() - validating");
		validate_RWaddress( arg0, "g_ascii_formatd - arg0 (buffer)");
		validate_NULL_TYPETYPE(  arg0, "g_ascii_formatd - arg0 (buffer)");
		validate_NULL_TYPETYPE(  arg1, "g_ascii_formatd - arg1 (buf_len)");
		validate_Rdaddress( arg2, "g_ascii_formatd - arg2 (format)");
		validate_NULL_TYPETYPE(  arg2, "g_ascii_formatd - arg2 (format)");
		validate_NULL_TYPETYPE(  arg3, "g_ascii_formatd - arg3 (d)");
	}
	ret_value = funcptr(arg0, arg1, arg2, arg3);
	__lsb_check_params = reset_flag;
	return ret_value;
}

