// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <glib-2.0/glib.h>
#undef g_queue_push_nth_link
static void(*funcptr) (GQueue * , gint , GList * ) = 0;

extern int __lsb_check_params;
void g_queue_push_nth_link (GQueue * arg0 , gint arg1 , GList * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for g_queue_push_nth_link()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_queue_push_nth_link");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_queue_push_nth_link. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "g_queue_push_nth_link() - validating");
		if( arg0 ) {
		validate_RWaddress( arg0, "g_queue_push_nth_link - arg0 (queue)");
		}
		validate_NULL_TYPETYPE(  arg0, "g_queue_push_nth_link - arg0 (queue)");
		validate_NULL_TYPETYPE(  arg1, "g_queue_push_nth_link - arg1 (n)");
		if( arg2 ) {
		validate_RWaddress( arg2, "g_queue_push_nth_link - arg2 (link_)");
		}
		validate_NULL_TYPETYPE(  arg2, "g_queue_push_nth_link - arg2 (link_)");
	}
	funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
}

