...
GVariant * g_variant_new_parsed_va (const gchar * arg0 , va_list * arg1 )
{
	...
	GVariant * ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_variant_new_parsed_va()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_variant_new_parsed_va");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_variant_new_parsed_va. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "g_variant_new_parsed_va() - validating");
		if( arg0 ) {
