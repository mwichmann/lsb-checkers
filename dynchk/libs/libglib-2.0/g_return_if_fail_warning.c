// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <glib-2.0/glib.h>
#undef g_return_if_fail_warning
static void(*funcptr) (const char * , const char * , const char * ) = 0;

extern int __lsb_check_params;
void g_return_if_fail_warning (const char * arg0 , const char * arg1 , const char * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for g_return_if_fail_warning()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_return_if_fail_warning");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_return_if_fail_warning. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "g_return_if_fail_warning() - validating");
		if( arg0 ) {
		validate_Rdaddress( arg0, "g_return_if_fail_warning - arg0 (log_domain)");
		}
		validate_NULL_TYPETYPE(  arg0, "g_return_if_fail_warning - arg0 (log_domain)");
		if( arg1 ) {
		validate_Rdaddress( arg1, "g_return_if_fail_warning - arg1 (pretty_function)");
		}
		validate_NULL_TYPETYPE(  arg1, "g_return_if_fail_warning - arg1 (pretty_function)");
		if( arg2 ) {
		validate_Rdaddress( arg2, "g_return_if_fail_warning - arg2 (expression)");
		}
		validate_NULL_TYPETYPE(  arg2, "g_return_if_fail_warning - arg2 (expression)");
	}
	funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
}

