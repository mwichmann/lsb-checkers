...
static gunichar2 *(*funcptr) (const gchar * , glong , glong * , glong * , GError * * ) = 0;

extern int __lsb_check_params;
gunichar2 * g_utf8_to_utf16 (const gchar * arg0 , glong arg1 , glong * arg2 , glong * arg3 , GError * * arg4 )
{
	int reset_flag = __lsb_check_params;
	gunichar2 * ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_utf8_to_utf16()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_utf8_to_utf16");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_utf8_to_utf16. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
