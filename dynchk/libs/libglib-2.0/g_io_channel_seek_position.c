...
static GIOStatus(*funcptr) (GIOChannel * , gint64 , GSeekType , GError * * ) = 0;

extern int __lsb_check_params;
GIOStatus g_io_channel_seek_position (GIOChannel * arg0 , gint64 arg1 , GSeekType arg2 , GError * * arg3 )
{
	int reset_flag = __lsb_check_params;
	GIOStatus ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_io_channel_seek_position()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_io_channel_seek_position");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_io_channel_seek_position. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
