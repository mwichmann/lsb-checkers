...
GNode * g_node_insert_after (GNode * arg0 , GNode * arg1 , GNode * arg2 )
{
	int reset_flag = __lsb_check_params;
	GNode * ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_node_insert_after()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_node_insert_after");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_node_insert_after. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
