// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <glib-2.0/glib.h>
#undef g_utf8_to_ucs4_fast
static gunichar *(*funcptr) (const gchar * , glong , glong * ) = 0;

extern int __lsb_check_params;
gunichar * g_utf8_to_ucs4_fast (const gchar * arg0 , glong arg1 , glong * arg2 )
{
	int reset_flag = __lsb_check_params;
	gunichar * ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_utf8_to_ucs4_fast()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_utf8_to_ucs4_fast");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_utf8_to_ucs4_fast. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "g_utf8_to_ucs4_fast() - validating");
		if( arg0 ) {
		validate_Rdaddress( arg0, "g_utf8_to_ucs4_fast - arg0 (str)");
		}
		validate_NULL_TYPETYPE(  arg0, "g_utf8_to_ucs4_fast - arg0 (str)");
		validate_NULL_TYPETYPE(  arg1, "g_utf8_to_ucs4_fast - arg1 (len)");
		if( arg2 ) {
		validate_RWaddress( arg2, "g_utf8_to_ucs4_fast - arg2 (items_written)");
		}
		validate_NULL_TYPETYPE(  arg2, "g_utf8_to_ucs4_fast - arg2 (items_written)");
	}
	ret_value = funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
	return ret_value;
}

