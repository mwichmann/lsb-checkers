...
void g_queue_insert_before (GQueue * arg0 , GList * arg1 , gpointer arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for g_queue_insert_before()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_queue_insert_before");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_queue_insert_before. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
