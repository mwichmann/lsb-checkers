...
static GIOStatus(*funcptr) (GIOChannel * , gboolean , GError * * ) = 0;

extern int __lsb_check_params;
GIOStatus g_io_channel_shutdown (GIOChannel * arg0 , gboolean arg1 , GError * * arg2 )
{
	int reset_flag = __lsb_check_params;
	GIOStatus ret_value  ;
	__lsb_output(4, "Invoking wrapper for g_io_channel_shutdown()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "g_io_channel_shutdown");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load g_io_channel_shutdown. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
