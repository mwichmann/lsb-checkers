...
void gtk_style_context_get_valist (GtkStyleContext * arg0 , GtkStateFlags arg1 , va_list arg2 )
{
	...
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_style_context_get_valist");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_style_context_get_valist. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "gtk_style_context_get_valist() - validating");
		if( arg0 ) {
