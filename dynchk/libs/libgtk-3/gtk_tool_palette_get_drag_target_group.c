...
const GtkTargetEntry * gtk_tool_palette_get_drag_target_group ()
{
	...
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tool_palette_get_drag_target_group. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "gtk_tool_palette_get_drag_target_group() - validating");
	}
	ret_value = funcptr();
