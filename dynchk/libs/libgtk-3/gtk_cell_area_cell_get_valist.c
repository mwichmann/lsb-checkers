...
void gtk_cell_area_cell_get_valist (GtkCellArea * arg0 , GtkCellRenderer * arg1 , const char * arg2 , struct __va_list_tag * arg3 )
{
	...
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_cell_area_cell_get_valist");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_cell_area_cell_get_valist. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "gtk_cell_area_cell_get_valist() - validating");
		if( arg0 ) {
		validate_RWaddress( arg0, "gtk_cell_area_cell_get_valist - arg0 (area)");
		}
