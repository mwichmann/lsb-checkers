...
void pango_xft_render_transformed (XftDraw * arg0 , XftColor * arg1 , PangoMatrix * arg2 , PangoFont * arg3 , PangoGlyphString * arg4 , int arg5 , int arg6 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for pango_xft_render_transformed()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "pango_xft_render_transformed");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load pango_xft_render_transformed. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
