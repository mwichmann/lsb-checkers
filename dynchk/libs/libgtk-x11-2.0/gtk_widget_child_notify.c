...
void gtk_widget_child_notify (GtkWidget * arg0 , const gchar * arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_widget_child_notify()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_widget_child_notify");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_widget_child_notify. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
