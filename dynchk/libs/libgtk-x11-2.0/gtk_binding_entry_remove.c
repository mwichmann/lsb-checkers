...
void gtk_binding_entry_remove (GtkBindingSet * arg0 , guint arg1 , GdkModifierType arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_binding_entry_remove()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_binding_entry_remove");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_binding_entry_remove. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
