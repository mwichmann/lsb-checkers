...
gboolean gtk_tree_view_row_expanded (GtkTreeView * arg0 , GtkTreePath * arg1 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gtk_tree_view_row_expanded()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tree_view_row_expanded");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tree_view_row_expanded. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
