...
static void(*funcptr) (GtkMenuShell * , GtkWidget * , gint ) = 0;

extern int __lsb_check_params;
void gtk_menu_shell_insert (GtkMenuShell * arg0 , GtkWidget * arg1 , gint arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_menu_shell_insert()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_menu_shell_insert");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_menu_shell_insert. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
