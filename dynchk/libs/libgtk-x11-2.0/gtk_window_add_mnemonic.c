...
void gtk_window_add_mnemonic (GtkWindow * arg0 , guint arg1 , GtkWidget * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_window_add_mnemonic()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_window_add_mnemonic");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_window_add_mnemonic. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
