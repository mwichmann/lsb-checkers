...
void gtk_font_button_set_show_size (GtkFontButton * arg0 , gboolean arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_font_button_set_show_size()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_font_button_set_show_size");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_font_button_set_show_size. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
