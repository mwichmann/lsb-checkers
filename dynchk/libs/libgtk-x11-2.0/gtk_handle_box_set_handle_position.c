...
void gtk_handle_box_set_handle_position (GtkHandleBox * arg0 , GtkPositionType arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_handle_box_set_handle_position()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_handle_box_set_handle_position");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_handle_box_set_handle_position. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
