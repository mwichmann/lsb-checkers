...
void gtk_widget_get_size_request (GtkWidget * arg0 , gint * arg1 , gint * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_widget_get_size_request()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_widget_get_size_request");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_widget_get_size_request. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
