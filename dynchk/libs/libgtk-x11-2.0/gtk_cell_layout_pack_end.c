...
void gtk_cell_layout_pack_end (GtkCellLayout * arg0 , GtkCellRenderer * arg1 , gboolean arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_cell_layout_pack_end()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_cell_layout_pack_end");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_cell_layout_pack_end. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
