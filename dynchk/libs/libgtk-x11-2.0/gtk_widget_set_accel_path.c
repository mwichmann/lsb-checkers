...
static void(*funcptr) (GtkWidget * , const gchar * , GtkAccelGroup * ) = 0;

extern int __lsb_check_params;
void gtk_widget_set_accel_path (GtkWidget * arg0 , const gchar * arg1 , GtkAccelGroup * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_widget_set_accel_path()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_widget_set_accel_path");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_widget_set_accel_path. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
