// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <glib-2.0/glib.h>
#include <gtk-2.0/gdk-pixbuf/gdk-pixbuf.h>
#include <gtk-2.0/gtk/gtk.h>
#undef gtk_icon_theme_add_builtin_icon
static void(*funcptr) (const gchar * , gint , GdkPixbuf * ) = 0;

extern int __lsb_check_params;
void gtk_icon_theme_add_builtin_icon (const gchar * arg0 , gint arg1 , GdkPixbuf * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_icon_theme_add_builtin_icon()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_icon_theme_add_builtin_icon");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_icon_theme_add_builtin_icon. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "gtk_icon_theme_add_builtin_icon() - validating");
		if( arg0 ) {
		validate_Rdaddress( arg0, "gtk_icon_theme_add_builtin_icon - arg0 (icon_name)");
		}
		validate_NULL_TYPETYPE(  arg0, "gtk_icon_theme_add_builtin_icon - arg0 (icon_name)");
		validate_NULL_TYPETYPE(  arg1, "gtk_icon_theme_add_builtin_icon - arg1 (size)");
		if( arg2 ) {
		validate_RWaddress( arg2, "gtk_icon_theme_add_builtin_icon - arg2 (pixbuf)");
		}
		validate_NULL_TYPETYPE(  arg2, "gtk_icon_theme_add_builtin_icon - arg2 (pixbuf)");
	}
	funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
}

