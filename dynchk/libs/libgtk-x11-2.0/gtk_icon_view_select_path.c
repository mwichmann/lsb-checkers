...
void gtk_icon_view_select_path (GtkIconView * arg0 , GtkTreePath * arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_icon_view_select_path()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_icon_view_select_path");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_icon_view_select_path. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
