...
void gtk_text_buffer_get_iter_at_offset (GtkTextBuffer * arg0 , GtkTextIter * arg1 , gint arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_text_buffer_get_iter_at_offset()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_text_buffer_get_iter_at_offset");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_text_buffer_get_iter_at_offset. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
