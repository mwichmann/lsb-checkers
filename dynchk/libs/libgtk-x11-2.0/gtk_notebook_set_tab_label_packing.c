...
void gtk_notebook_set_tab_label_packing (GtkNotebook * arg0 , GtkWidget * arg1 , gboolean arg2 , gboolean arg3 , GtkPackType arg4 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_notebook_set_tab_label_packing()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_notebook_set_tab_label_packing");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_notebook_set_tab_label_packing. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		...
		validate_NULL_TYPETYPE(  arg4, "gtk_notebook_set_tab_label_packing - arg4 (pack_type)");
	}
