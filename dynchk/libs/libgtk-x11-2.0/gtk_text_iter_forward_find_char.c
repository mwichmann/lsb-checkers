...
gboolean gtk_text_iter_forward_find_char (GtkTextIter * arg0 , GtkTextCharPredicate arg1 , gpointer arg2 , const GtkTextIter * arg3 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gtk_text_iter_forward_find_char()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_text_iter_forward_find_char");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_text_iter_forward_find_char. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
