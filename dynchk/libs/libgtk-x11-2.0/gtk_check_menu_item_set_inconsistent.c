...
void gtk_check_menu_item_set_inconsistent (GtkCheckMenuItem * arg0 , gboolean arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_check_menu_item_set_inconsistent()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_check_menu_item_set_inconsistent");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_check_menu_item_set_inconsistent. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
