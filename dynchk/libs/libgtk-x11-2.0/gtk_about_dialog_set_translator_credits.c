...
void gtk_about_dialog_set_translator_credits (GtkAboutDialog * arg0 , const gchar * arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_about_dialog_set_translator_credits()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_about_dialog_set_translator_credits");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_about_dialog_set_translator_credits. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
