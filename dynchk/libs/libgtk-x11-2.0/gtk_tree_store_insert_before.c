...
void gtk_tree_store_insert_before (GtkTreeStore * arg0 , GtkTreeIter * arg1 , GtkTreeIter * arg2 , GtkTreeIter * arg3 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_tree_store_insert_before()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tree_store_insert_before");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tree_store_insert_before. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
