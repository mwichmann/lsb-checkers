...
static gboolean(*funcptr) (GtkTreeModel * , GtkTreeIter * , GtkTreePath * ) = 0;

extern int __lsb_check_params;
gboolean gtk_tree_model_get_iter (GtkTreeModel * arg0 , GtkTreeIter * arg1 , GtkTreePath * arg2 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gtk_tree_model_get_iter()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tree_model_get_iter");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tree_model_get_iter. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
