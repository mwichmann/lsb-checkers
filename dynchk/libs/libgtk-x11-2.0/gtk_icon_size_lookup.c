...
static gboolean(*funcptr) (GtkIconSize , gint * , gint * ) = 0;

extern int __lsb_check_params;
gboolean gtk_icon_size_lookup (GtkIconSize arg0 , gint * arg1 , gint * arg2 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gtk_icon_size_lookup()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_icon_size_lookup");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_icon_size_lookup. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
