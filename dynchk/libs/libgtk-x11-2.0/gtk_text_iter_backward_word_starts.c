...
gboolean gtk_text_iter_backward_word_starts (GtkTextIter * arg0 , gint arg1 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gtk_text_iter_backward_word_starts()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_text_iter_backward_word_starts");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_text_iter_backward_word_starts. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
