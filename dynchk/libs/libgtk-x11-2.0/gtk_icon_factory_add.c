// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <gtk-2.0/gtk/gtk.h>
#include <glib-2.0/glib.h>
#undef gtk_icon_factory_add
static void(*funcptr) (GtkIconFactory * , const gchar * , GtkIconSet * ) = 0;

extern int __lsb_check_params;
void gtk_icon_factory_add (GtkIconFactory * arg0 , const gchar * arg1 , GtkIconSet * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_icon_factory_add()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_icon_factory_add");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_icon_factory_add. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "gtk_icon_factory_add() - validating");
		validate_RWaddress( arg0, "gtk_icon_factory_add - arg0 (factory)");
		validate_NULL_TYPETYPE(  arg0, "gtk_icon_factory_add - arg0 (factory)");
		validate_Rdaddress( arg1, "gtk_icon_factory_add - arg1 (stock_id)");
		validate_NULL_TYPETYPE(  arg1, "gtk_icon_factory_add - arg1 (stock_id)");
		validate_RWaddress( arg2, "gtk_icon_factory_add - arg2 (icon_set)");
		validate_NULL_TYPETYPE(  arg2, "gtk_icon_factory_add - arg2 (icon_set)");
	}
	funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
}

