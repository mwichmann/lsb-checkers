...
static gboolean(*funcptr) (GtkTextBuffer * , GtkTextIter * , GtkTextIter * , gboolean ) = 0;

extern int __lsb_check_params;
gboolean gtk_text_buffer_delete_interactive (GtkTextBuffer * arg0 , GtkTextIter * arg1 , GtkTextIter * arg2 , gboolean arg3 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gtk_text_buffer_delete_interactive()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_text_buffer_delete_interactive");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_text_buffer_delete_interactive. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
