...
void gtk_about_dialog_set_logo_icon_name (GtkAboutDialog * arg0 , const gchar * arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_about_dialog_set_logo_icon_name()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_about_dialog_set_logo_icon_name");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_about_dialog_set_logo_icon_name. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
