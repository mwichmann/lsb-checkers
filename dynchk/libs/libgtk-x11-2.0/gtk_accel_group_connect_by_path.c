// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <gtk-2.0/gtk/gtk.h>
#include <glib-2.0/glib.h>
#include <glib-2.0/glib-object.h>
#undef gtk_accel_group_connect_by_path
static void(*funcptr) (GtkAccelGroup * , const gchar * , GClosure * ) = 0;

extern int __lsb_check_params;
void gtk_accel_group_connect_by_path (GtkAccelGroup * arg0 , const gchar * arg1 , GClosure * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_accel_group_connect_by_path()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_accel_group_connect_by_path");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_accel_group_connect_by_path. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "gtk_accel_group_connect_by_path() - validating");
		if( arg0 ) {
		validate_RWaddress( arg0, "gtk_accel_group_connect_by_path - arg0 (accel_group)");
		}
		validate_NULL_TYPETYPE(  arg0, "gtk_accel_group_connect_by_path - arg0 (accel_group)");
		if( arg1 ) {
		validate_Rdaddress( arg1, "gtk_accel_group_connect_by_path - arg1 (accel_path)");
		}
		validate_NULL_TYPETYPE(  arg1, "gtk_accel_group_connect_by_path - arg1 (accel_path)");
		if( arg2 ) {
		validate_RWaddress( arg2, "gtk_accel_group_connect_by_path - arg2 (closure)");
		}
		validate_NULL_TYPETYPE(  arg2, "gtk_accel_group_connect_by_path - arg2 (closure)");
	}
	funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
}

