...
gboolean gtk_tree_set_row_drag_data (GtkSelectionData * arg0 , GtkTreeModel * arg1 , GtkTreePath * arg2 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gtk_tree_set_row_drag_data()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tree_set_row_drag_data");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tree_set_row_drag_data. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
