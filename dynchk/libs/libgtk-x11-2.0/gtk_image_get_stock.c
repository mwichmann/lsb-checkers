...
static void(*funcptr) (GtkImage * , gchar * * , GtkIconSize * ) = 0;

extern int __lsb_check_params;
void gtk_image_get_stock (GtkImage * arg0 , gchar * * arg1 , GtkIconSize * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_image_get_stock()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_image_get_stock");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_image_get_stock. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
