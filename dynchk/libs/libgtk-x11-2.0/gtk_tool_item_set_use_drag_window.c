...
void gtk_tool_item_set_use_drag_window (GtkToolItem * arg0 , gboolean arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_tool_item_set_use_drag_window()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tool_item_set_use_drag_window");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tool_item_set_use_drag_window. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
