...
static void(*funcptr) (GtkToolItem * , const gchar * , GtkWidget * ) = 0;

extern int __lsb_check_params;
void gtk_tool_item_set_proxy_menu_item (GtkToolItem * arg0 , const gchar * arg1 , GtkWidget * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_tool_item_set_proxy_menu_item()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tool_item_set_proxy_menu_item");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tool_item_set_proxy_menu_item. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
