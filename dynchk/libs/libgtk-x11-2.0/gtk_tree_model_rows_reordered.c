...
static void(*funcptr) (GtkTreeModel * , GtkTreePath * , GtkTreeIter * , gint * ) = 0;

extern int __lsb_check_params;
void gtk_tree_model_rows_reordered (GtkTreeModel * arg0 , GtkTreePath * arg1 , GtkTreeIter * arg2 , gint * arg3 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_tree_model_rows_reordered()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tree_model_rows_reordered");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tree_model_rows_reordered. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
