...
void gtk_tree_view_column_set_min_width (GtkTreeViewColumn * arg0 , gint arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_tree_view_column_set_min_width()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tree_view_column_set_min_width");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tree_view_column_set_min_width. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
