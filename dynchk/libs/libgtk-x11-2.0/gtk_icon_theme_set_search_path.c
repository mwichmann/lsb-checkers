...
static void(*funcptr) (GtkIconTheme * , const gchar * * , gint ) = 0;

extern int __lsb_check_params;
void gtk_icon_theme_set_search_path (GtkIconTheme * arg0 , const gchar * * arg1 , gint arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_icon_theme_set_search_path()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_icon_theme_set_search_path");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_icon_theme_set_search_path. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
