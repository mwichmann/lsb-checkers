...
void gtk_tree_selection_unselect_range (GtkTreeSelection * arg0 , GtkTreePath * arg1 , GtkTreePath * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_tree_selection_unselect_range()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_tree_selection_unselect_range");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_tree_selection_unselect_range. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
