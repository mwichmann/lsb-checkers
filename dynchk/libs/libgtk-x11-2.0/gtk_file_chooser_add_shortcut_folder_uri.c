...
gboolean gtk_file_chooser_add_shortcut_folder_uri (GtkFileChooser * arg0 , const char * arg1 , GError * * arg2 )
{
	int reset_flag = __lsb_check_params;
	gboolean ret_value  ;
	__lsb_output(4, "Invoking wrapper for gtk_file_chooser_add_shortcut_folder_uri()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_file_chooser_add_shortcut_folder_uri");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_file_chooser_add_shortcut_folder_uri. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
