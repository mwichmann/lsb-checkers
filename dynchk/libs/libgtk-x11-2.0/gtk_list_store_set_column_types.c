// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <gtk-2.0/gtk/gtk.h>
#include <glib-2.0/glib.h>
#include <glib-2.0/glib-object.h>
#undef gtk_list_store_set_column_types
static void(*funcptr) (GtkListStore * , gint , GType * ) = 0;

extern int __lsb_check_params;
void gtk_list_store_set_column_types (GtkListStore * arg0 , gint arg1 , GType * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_list_store_set_column_types()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_list_store_set_column_types");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_list_store_set_column_types. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "gtk_list_store_set_column_types() - validating");
		validate_RWaddress( arg0, "gtk_list_store_set_column_types - arg0 (list_store)");
		validate_NULL_TYPETYPE(  arg0, "gtk_list_store_set_column_types - arg0 (list_store)");
		validate_NULL_TYPETYPE(  arg1, "gtk_list_store_set_column_types - arg1 (n_columns)");
		validate_RWaddress( arg2, "gtk_list_store_set_column_types - arg2 (types)");
		validate_NULL_TYPETYPE(  arg2, "gtk_list_store_set_column_types - arg2 (types)");
	}
	funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
}

