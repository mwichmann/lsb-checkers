...
void gtk_accel_map_add_entry (const gchar * arg0 , guint arg1 , GdkModifierType arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_accel_map_add_entry()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_accel_map_add_entry");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_accel_map_add_entry. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
