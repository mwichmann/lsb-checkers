...
void gtk_text_buffer_remove_all_tags (GtkTextBuffer * arg0 , const GtkTextIter * arg1 , const GtkTextIter * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_text_buffer_remove_all_tags()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_text_buffer_remove_all_tags");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_text_buffer_remove_all_tags. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
