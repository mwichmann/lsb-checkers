...
void gtk_widget_set_double_buffered (GtkWidget * arg0 , gboolean arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_widget_set_double_buffered()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_widget_set_double_buffered");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_widget_set_double_buffered. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
