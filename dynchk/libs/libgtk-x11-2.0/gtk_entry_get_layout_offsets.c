...
static void(*funcptr) (GtkEntry * , gint * , gint * ) = 0;

extern int __lsb_check_params;
void gtk_entry_get_layout_offsets (GtkEntry * arg0 , gint * arg1 , gint * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_entry_get_layout_offsets()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_entry_get_layout_offsets");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_entry_get_layout_offsets. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
