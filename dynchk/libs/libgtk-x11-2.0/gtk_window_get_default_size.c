...
void gtk_window_get_default_size (GtkWindow * arg0 , gint * arg1 , gint * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_window_get_default_size()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_window_get_default_size");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_window_get_default_size. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
