...
void gtk_notebook_set_tab_label_text (GtkNotebook * arg0 , GtkWidget * arg1 , const gchar * arg2 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_notebook_set_tab_label_text()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_notebook_set_tab_label_text");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_notebook_set_tab_label_text. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
