...
void gtk_about_dialog_set_website_label (GtkAboutDialog * arg0 , const gchar * arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_about_dialog_set_website_label()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_about_dialog_set_website_label");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_about_dialog_set_website_label. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
