...
static void(*funcptr) (GtkTextBuffer * , GtkTextIter * , const GtkTextIter * , const GtkTextIter * ) = 0;

extern int __lsb_check_params;
void gtk_text_buffer_insert_range (GtkTextBuffer * arg0 , GtkTextIter * arg1 , const GtkTextIter * arg2 , const GtkTextIter * arg3 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_text_buffer_insert_range()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_text_buffer_insert_range");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_text_buffer_insert_range. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
