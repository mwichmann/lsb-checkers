...
static void(*funcptr) (GtkActionGroup * , const gchar * ) = 0;

extern int __lsb_check_params;
void gtk_action_group_set_translation_domain (GtkActionGroup * arg0 , const gchar * arg1 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_action_group_set_translation_domain()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_action_group_set_translation_domain");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_action_group_set_translation_domain. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
