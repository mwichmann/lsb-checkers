...
void gtk_widget_shape_combine_mask (GtkWidget * arg0 , GdkBitmap * arg1 , gint arg2 , gint arg3 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for gtk_widget_shape_combine_mask()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gtk_widget_shape_combine_mask");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gtk_widget_shape_combine_mask. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
