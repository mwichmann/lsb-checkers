// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <zlib.h>
#undef inflateSetDictionary
static int(*funcptr) (z_streamp , const Bytef * , uInt ) = 0;

extern int __lsb_check_params;
int inflateSetDictionary (z_streamp arg0 , const Bytef * arg1 , uInt arg2 )
{
	int reset_flag = __lsb_check_params;
	int ret_value  ;
	__lsb_output(4, "Invoking wrapper for inflateSetDictionary()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "inflateSetDictionary");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load inflateSetDictionary. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "inflateSetDictionary() - validating");
		validate_NULL_TYPETYPE(  arg0, "inflateSetDictionary - arg0 (strm)");
		if( arg1 ) {
		validate_Rdaddress( arg1, "inflateSetDictionary - arg1 (dictionary)");
		}
		validate_NULL_TYPETYPE(  arg1, "inflateSetDictionary - arg1 (dictionary)");
		validate_NULL_TYPETYPE(  arg2, "inflateSetDictionary - arg2 (dictLength)");
	}
	ret_value = funcptr(arg0, arg1, arg2);
	__lsb_check_params = reset_flag;
	return ret_value;
}

