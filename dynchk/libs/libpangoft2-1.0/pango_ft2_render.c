...
void pango_ft2_render (FT_Bitmap * arg0 , PangoFont * arg1 , PangoGlyphString * arg2 , int arg3 , int arg4 )
{
	...
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "pango_ft2_render");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load pango_ft2_render. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
