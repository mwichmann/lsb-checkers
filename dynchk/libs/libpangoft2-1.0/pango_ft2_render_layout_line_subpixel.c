...
#undef pango_ft2_render_layout_line_subpixel
static void(*funcptr) (FT_Bitmap * , PangoLayoutLine * , int , int ) = 0;

extern int __lsb_check_params;
void pango_ft2_render_layout_line_subpixel (FT_Bitmap * arg0 , PangoLayoutLine * arg1 , int arg2 , int arg3 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for pango_ft2_render_layout_line_subpixel()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "pango_ft2_render_layout_line_subpixel");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load pango_ft2_render_layout_line_subpixel. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
