...
KeySym XtGetActionKeysym (XEvent * arg0 , Modifiers * arg1 )
{
	...
	KeySym ret_value  ;
	__lsb_output(4, "Invoking wrapper for XtGetActionKeysym()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "XtGetActionKeysym");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load XtGetActionKeysym. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "XtGetActionKeysym() - validating");
		if( arg0 ) {
