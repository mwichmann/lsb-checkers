...
GdkPixbuf * gdk_pixbuf_new_from_file_at_scale (const char * arg0 , int arg1 , int arg2 , gboolean arg3 , GError * * arg4 )
{
	int reset_flag = __lsb_check_params;
	GdkPixbuf * ret_value  ;
	__lsb_output(4, "Invoking wrapper for gdk_pixbuf_new_from_file_at_scale()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "gdk_pixbuf_new_from_file_at_scale");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load gdk_pixbuf_new_from_file_at_scale. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
