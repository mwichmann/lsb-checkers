// Generated by gen_lib.pl

#include "../../tests/type_tests.h"
#include "../../misc/lsb_output.h"
#include "stdlib.h"
#include <tiffio.h>
#undef TIFFSwabDouble
static void(*funcptr) (double * ) = 0;

extern int __lsb_check_params;
void TIFFSwabDouble (double * arg0 )
{
	int reset_flag = __lsb_check_params;
	__lsb_output(4, "Invoking wrapper for TIFFSwabDouble()");
	if(!funcptr)
		funcptr = dlsym(RTLD_NEXT, "TIFFSwabDouble");
	if(!funcptr) {
		__lsb_output(-1, "Failed to load TIFFSwabDouble. Probably the library was loaded using dlopen, we don't support this at the moment.");
		exit(1);
	}
	if(__lsb_check_params)
	{
		__lsb_check_params=0;
		__lsb_output(4, "TIFFSwabDouble() - validating");
		if( arg0 ) {
		validate_RWaddress( arg0, "TIFFSwabDouble - arg0");
		}
		validate_NULL_TYPETYPE(  arg0, "TIFFSwabDouble - arg0");
	}
	funcptr(arg0);
	__lsb_check_params = reset_flag;
}

