...
WHERE Ttype = "Struct"
  ...
  AND Hstd = "Yes"
  AND (Tarch = 1 OR Tarch = 2)
GROUP BY Tid'
							 )#Tarch constrains to my system-make configurable!
...
STRUCT: while(my ($struct_name, $struct_id, $struct_header) = $struct_q->fetchrow_array())
{
	...
	next STRUCT if($struct_header =~ /\AX11/);  #skip X - incomplete data.

	$member_q->execute($struct_id)
		or die "Couldn't execute member query: " . DBI->errstr;
	open(OUTFILE, '>struct/validate_struct_' . $struct_name . '.c')
	...
	print OUTFILE "void validate_struct_".$struct_name."(struct ".$struct_name." * input)\n";
	print OUTFILE "{\n";
