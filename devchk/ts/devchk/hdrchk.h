...
#define CheckTypeSize(type,size,tid,aid,appearedin,withdrawnin,basetype,attribute) \
    ...
    if( sizeof(type) != size ) { \
        ...
        Msg("Ping-pong check:%d;%d;%d;%d;" #appearedin ";" #withdrawnin ";%d;" #attribute ";" #type "\n", tid, aid, sizeof(type), size, basetype); \
...
#define CheckMethoddef(funcname,dbfunctype) \
{ \
    dbfunctype ptr = &funcname; \
}
