...
/*XXX this routine desperately need to be broken up into readable chunks
 * it's nearly 600 lines at the moment, and excess indentation makes
 * readability even worse
 */
void
...
#define TMP_STRING_SIZE (400)
	...
	*/
	...
	if (filesizes) {
		...
		fprintf(stderr,
			"Filesize (%d) for %s not that same a specified in RPMTAG_FILESIZES (%d)\n",
	...
	if (filetimes && (ftime != filetimes[fileindex])) {
	    fprintf(stderr,
		    "File time  (%x) for %s not the same as specified in RPMTAG_FILEMTIMES (%x)\n",
	...
	if (fileinodes && (fino != fileinodes[fileindex])) {
	    fprintf(stderr,
		    "File inode  (%x) for %s not the same as specified in RPMTAG_FILEINODES (%x)\n",
		...
		if ((elfFile =
			...
			if ((elf_type == ET_EXEC)
				...
				elfFiles = (ArchiveElfInfo **) 
				     calloc(100, sizeof (ArchiveElfInfo *));
			    }
			    ...
			    elfFiles[numArchiveElfInfo - 1] = (ArchiveElfInfo *)
					    calloc(1, sizeof (ArchiveElfInfo));
	...
	if (flink == 1 && flinktos) {
		...
		fprintf(stderr,
			"File link expected, but no FILELINKTOS entry\n");
	    }
	    if (S_ISREG(mode) && flink == 1 && *flinktos) {
		fprintf(stderr,
	...
	fprintf(stderr, "Archive size (%d) does ",
		endoffset - startoffset);
